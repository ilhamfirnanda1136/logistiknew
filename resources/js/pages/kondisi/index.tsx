...
import type { Kondisi, KondisiFormData } from '@/types/kondisi';

const EMPTY_FORM: KondisiFormData = {
    nama_kondisi: '',
    keterangan: '',
};
...
interface FormDialogProps {
    open: boolean;
    onClose: () => void;
    editData: Kondisi | null;
    onSuccess: () => void;
}

function FormDialog({ open, onClose, editData, onSuccess }: FormDialogProps) {
    const [form, setForm] = useState<KondisiFormData>(EMPTY_FORM);
    const [errors, setErrors] = useState<Partial<Record<keyof KondisiFormData, string>>>({});
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (editData) {
            setForm({
                nama_kondisi: editData.nama_kondisi,
    ...
    const handleChange = (field: keyof KondisiFormData, value: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editData) {
                await apiPut(`/kondisi/${editData.id}`, form);
                toast.success('Kondisi berhasil diperbarui!');
            } else {
                await apiPost('/kondisi', form);
            ...
            if (apiErr?.status === 422 && apiErr?.data?.errors) {
                const mapped: Partial<Record<keyof KondisiFormData, string>> = {};
                Object.entries(apiErr.data.errors).forEach(([key, msgs]) => {
                    mapped[key as keyof KondisiFormData] = msgs[0];
                });
                setErrors(mapped);
            } else {
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-base">
                        ...
                        {editData ? 'Edit Kondisi' : 'Form Simpan Kondisi'}
                    </DialogTitle>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                    <div className="space-y-1.5">
                        <Label htmlFor="nama_kondisi">
                            Kondisi <span className="text-destructive">*</span>
                        </Label>
                        <Input
                            id="nama_kondisi"
                            value={form.nama_kondisi}
                            onChange={(e) => handleChange('nama_kondisi', e.target.value)}
                            placeholder="Contoh: Hilang, Rusak, Tersedia"
                            autoFocus
                            className={errors.nama_kondisi ? 'border-destructive focus-visible:ring-destructive' : ''}
                        />
                        {errors.nama_kondisi && (
                            <p className="text-xs text-destructive">{errors.nama_kondisi}</p>
                        )}
                    </div>
...
function DeleteDialog({ open, kondisi, onClose, onSuccess }: DeleteDialogProps) {
    const [loading, setLoading] = useState(false);
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            await apiDelete(`/kondisi/${kondisi.id}`);
            toast.success('Kondisi berhasil dihapus.');
            onSuccess();
            onClose();
        } catch {
            toast.error('Gagal menghapus kondisi. Silakan coba lagi.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <DialogContent className="sm:max-w-sm">
                ...
                <div className="space-y-3 py-2">
                    <p className="text-sm text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">"{kondisi?.nama_kondisi}"</span>?
                    </p>
...
export default function KondisiIndex() {
    const [refreshTrigger, setRefreshTrigger] = useState(0);

    const [formOpen, setFormOpen] = useState(false);
    const [editData, setEditData] = useState<Kondisi | null>(null);
    ...
    const openEdit = (row: Kondisi) => {
        setEditData(row);
        setFormOpen(true);
    };

    const openDelete = (row: Kondisi) => {
        setDeleteTarget(row);
        setDeleteOpen(true);
    };
    ...
    const columns: ColumnDef<Kondisi>[] = [
        ...
        },
        {
            ...
            label: 'KONDISI',
            sortable: true,
            render: (row) => <span className="font-medium">{row.nama_kondisi}</span>,
        },
        {
            id: 'keterangan',
            label: 'KETERANGAN',
        ...
        <>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Kondisi</h1>
                        <p className="text-sm text-muted-foreground mt-0.5">
                            Kelola data master kondisi asset.
                        </p>
                    </div>
                    <Button
                        onClick={openCreate}
                ...
                <ServerDataTable<Kondisi>
                    endpoint="/kondisi/datatable"
                    columns={columns}
                    refreshTrigger={refreshTrigger}
                    emptyStateIcon={<ClipboardList className="h-7 w-7" />}
                    emptyStateTitle="Tidak ada data kondisi"
                    emptyStateMessage="Belum ada kondisi yang ditambahkan. Silakan klik Tambah."
                />
            </div>
