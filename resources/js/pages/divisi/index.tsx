...
import type { Divisi, DivisiFormData } from '@/types/divisi';

const EMPTY_FORM: DivisiFormData = {
    nama_divisi: '',
    keterangan: '',
    is_active: true,
};
...
interface FormDialogProps {
    open: boolean;
    onClose: () => void;
    editData: Divisi | null;
    onSuccess: () => void;
}

function FormDialog({ open, onClose, editData, onSuccess }: FormDialogProps) {
    const [form, setForm] = useState<DivisiFormData>(EMPTY_FORM);
    const [errors, setErrors] = useState<Partial<Record<keyof DivisiFormData, string>>>({});
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (editData) {
            setForm({
                nama_divisi: editData.nama_divisi,
    ...
    const handleChange = (field: keyof DivisiFormData, value: string | boolean) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editData) {
                await apiPut(`/divisi/${editData.id}`, form);
                toast.success('Divisi berhasil diperbarui!');
            } else {
                await apiPost('/divisi', form);
            ...
            if (apiErr?.status === 422 && apiErr?.data?.errors) {
                const mapped: Partial<Record<keyof DivisiFormData, string>> = {};
                Object.entries(apiErr.data.errors).forEach(([key, msgs]) => {
                    mapped[key as keyof DivisiFormData] = msgs[0];
                });
                setErrors(mapped);
            } else {
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-base">
                        ...
                        {editData ? 'Edit Divisi' : 'Form Tambah Divisi'}
                    </DialogTitle>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                    <div className="space-y-1.5">
                        <Label htmlFor="nama_divisi">
                            Divisi <span className="text-destructive">*</span>
                        </Label>
                        <Input
                            id="nama_divisi"
                            value={form.nama_divisi}
                            onChange={(e) => handleChange('nama_divisi', e.target.value)}
                            placeholder="Nama divisi..."
                            autoFocus
                            className={errors.nama_divisi ? 'border-destructive focus-visible:ring-destructive' : ''}
                        />
                        {errors.nama_divisi && (
                            <p className="text-xs text-destructive">{errors.nama_divisi}</p>
                        )}
                    </div>
...
function DeleteDialog({ open, divisi, onClose, onSuccess }: DeleteDialogProps) {
    const [loading, setLoading] = useState(false);
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            await apiDelete(`/divisi/${divisi.id}`);
            toast.success('Divisi berhasil dihapus.');
            onSuccess();
            onClose();
        } catch {
            toast.error('Gagal menghapus divisi. Silakan coba lagi.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <DialogContent className="sm:max-w-sm">
                ...
                <div className="space-y-3 py-2">
                    <p className="text-sm text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">"{divisi?.nama_divisi}"</span>?
                    </p>
...
export default function DivisiIndex() {
    ...
    const openEdit = (row: Divisi) => {
        setEditData(row);
        setFormOpen(true);
    };

    const openDelete = (row: Divisi) => {
        setDeleteTarget(row);
        setDeleteOpen(true);
    };
    ...
    const columns: ColumnDef<Divisi>[] = [
        ...
        },
        {
            ...
            label: 'DIVISI',
            sortable: true,
            render: (row) => <span className="font-medium">{row.nama_divisi}</span>
        },
        {
            id: 'keterangan',
            label: 'KETERANGAN',
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">
                            Kelola data divisi dalam organisasi.
                        </p>
                    </div>
                    <Button
                        onClick={openCreate}
                ...
                <ServerDataTable<Divisi>
                    endpoint="/divisi/datatable"
                    columns={columns}
                    refreshTrigger={refreshTrigger}
                    emptyStateIcon={<Building2 className="h-7 w-7" />}
                    emptyStateTitle="Tidak ada data"
                    emptyStateMessage="Belum ada divisi yang ditambahkan."
                />
            </div>
