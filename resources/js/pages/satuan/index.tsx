...
import type { Satuan, SatuanFormData } from '@/types/satuan';

const EMPTY_FORM: SatuanFormData = {
    nama_satuan: '',
    keterangan: '',
};
...
interface FormDialogProps {
    open: boolean;
    onClose: () => void;
    editData: Satuan | null;
    onSuccess: () => void;
}

function FormDialog({ open, onClose, editData, onSuccess }: FormDialogProps) {
    const [form, setForm] = useState<SatuanFormData>(EMPTY_FORM);
    const [errors, setErrors] = useState<Partial<Record<keyof SatuanFormData, string>>>({});
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        if (editData) {
            setForm({
                nama_satuan: editData.nama_satuan,
    ...
    const handleChange = (field: keyof SatuanFormData, value: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editData) {
                await apiPut(`/barang/satuan/${editData.id}`, form);
                toast.success('Satuan berhasil diperbarui!');
            } else {
                await apiPost('/barang/satuan', form);
            ...
            if (apiErr?.status === 422 && apiErr?.data?.errors) {
                const mapped: Partial<Record<keyof SatuanFormData, string>> = {};
                Object.entries(apiErr.data.errors).forEach(([key, msgs]) => {
                    mapped[key as keyof SatuanFormData] = msgs[0];
                });
                setErrors(mapped);
            } else {
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-base">
                        ...
                        {editData ? 'Edit Satuan' : 'Form Simpan Satuan'}
                    </DialogTitle>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                    <div className="space-y-1.5">
                        ...
                        <Input
                            ...
                            onChange={(e) => handleChange('nama_satuan', e.target.value)}
                            placeholder="Contoh: kg, pcs, liter"
                            autoFocus
                            className={errors.nama_satuan ? 'border-destructive focus-visible:ring-destructive' : ''}
                        />
                        {errors.nama_satuan && (
                            <p className="text-xs text-destructive">{errors.nama_satuan}</p>
                        )}
                    </div>
...
function DeleteDialog({ open, satuan, onClose, onSuccess }: DeleteDialogProps) {
    const [loading, setLoading] = useState(false);
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            await apiDelete(`/barang/satuan/${satuan.id}`);
            toast.success('Satuan berhasil dihapus.');
            onSuccess();
            onClose();
        } catch {
            toast.error('Gagal menghapus satuan. Silakan coba lagi.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <DialogContent className="sm:max-w-sm">
                ...
                <div className="space-y-3 py-2">
                    <p className="text-sm text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">"{satuan?.nama_satuan}"</span>?
                    </p>
...
export default function SatuanIndex() {
    ...
    const [editData, setEditData] = useState<Satuan | null>(null);
    ...
    const columns: ColumnDef<Satuan>[] = [
        ...
        },
        {
            ...
            label: 'SATUAN',
            sortable: true,
            render: (row) => <span className="font-medium">{row.nama_satuan}</span>,
        },
        {
            id: 'keterangan',
            label: 'KETERANGAN',
        ...
        <>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Satuan</h1>
                        <p className="text-sm text-muted-foreground mt-0.5">
                            Kelola data master satuan barang.
                        </p>
                    </div>
                    <Button
                        onClick={openCreate}
                ...
                <ServerDataTable<Satuan>
                    endpoint="/barang/satuan/datatable"
                    columns={columns}
                    refreshTrigger={refreshTrigger}
                    emptyStateIcon={<Tag className="h-7 w-7" />}
                    emptyStateTitle="Tidak ada data satuan"
                    emptyStateMessage="Belum ada satuan yang ditambahkan. Silakan klik Tambah."
                />
            </div>
...
SatuanIndex.layout = {
    breadcrumbs: [
        ...
        { title: 'Satuan', href: '/barang/satuan' },
    ],
};
