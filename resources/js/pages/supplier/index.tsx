...
function FormDialog({ open, onClose, editData, onSuccess }: FormDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editData) {
                await apiPut(`/supplier/${editData.id}`, form);
                toast.success('Supplier berhasil diperbarui!');
            } else {
                await apiPost('/supplier', form);
            ...
            if (apiErr?.status === 422 && apiErr?.data?.errors) {
                const mapped: Partial<Record<keyof SupplierFormData, string>> = {};
                Object.entries(apiErr.data.errors).forEach(([key, msgs]) => {
                    mapped[key as keyof SupplierFormData] = msgs[0];
                });
                setErrors(mapped);
            } else {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                onChange={(e) => handleChange('nama_supplier', e.target.value)}
                                autoFocus
                                className={errors.nama_supplier ? 'border-destructive focus-visible:ring-destructive' : ''}
                            />
                            {errors.nama_supplier && (
                                <p className="text-xs text-destructive">{errors.nama_supplier}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                className={errors.no_telepon ? 'border-destructive focus-visible:ring-destructive' : ''}
                            />
                            {errors.no_telepon && (
                                <p className="text-xs text-destructive">{errors.no_telepon}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <textarea
                                ...
                                rows={3}
                                className={`w-full rounded-md border bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 resize-none ${errors.alamat ? 'border-destructive' : 'border-input'}`}
                            />
                            {errors.alamat && (
...
function DeleteDialog({ open, supplier, onClose, onSuccess }: DeleteDialogProps) {
    const [loading, setLoading] = useState(false);
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            await apiDelete(`/supplier/${supplier.id}`);
            toast.success('Supplier berhasil dihapus.');
            onSuccess();
            onClose();
        } catch {
            toast.error('Gagal menghapus supplier. Silakan coba lagi.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <DialogContent className="sm:max-w-sm">
                ...
                <div className="space-y-3 py-2">
                    <p className="text-sm text-muted-foreground">
                        ...
                        <span className="font-semibold text-foreground">"{supplier?.nama_supplier}"</span>?
                    </p>
...
export default function SupplierIndex() {
    ...
    const columns: ColumnDef<Supplier>[] = [
        ...
        },
        {
            ...
            label: 'SUPPLIER',
            sortable: true,
            render: (row) => <span className="font-medium">{row.nama_supplier}</span>
        },
        {
            id: 'no_telepon',
            label: 'TELEPON',
            sortable: true,
            render: (row) => <span className="text-sm">{row.no_telepon ?? <span className="italic opacity-50">-</span>}</span>
        },
        {
            id: 'alamat',
            label: 'ALAMAT',
            render: (row) => (
                <span className="text-muted-foreground text-xs line-clamp-2 max-w-[250px]">
                    {row.alamat ?? <span className="italic">-</span>}
                </span>
            )
        },
        {
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                ...
                <ServerDataTable<Supplier>
                    endpoint="/supplier/datatable"
                    columns={columns}
                    refreshTrigger={refreshTrigger}
                    emptyStateIcon={<Truck className="h-7 w-7" />}
                    emptyStateTitle="Tidak ada data supplier"
                    emptyStateMessage="Belum ada supplier yang ditambahkan."
                />
            </div>
