...
import type { Manager, ManagerFormData } from '@/types/manager';
...
interface FormDialogProps {
    open: boolean;
    onClose: () => void;
    editData: Manager | null;
    onSuccess: () => void;
}

function FormDialog({ open, onClose, editData, onSuccess }: FormDialogProps) {
    const [form, setForm] = useState<ManagerFormData>(EMPTY_FORM);
    ...
    useEffect(() => {
        if (editData) {
            setForm({
                nama_manager: editData.nama_manager,
                npp: editData.npp ?? '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editData) {
                await apiPut(`/manager/${editData.id}`, form);
                toast.success('Manager berhasil diperbarui!');
            } else {
                await apiPost('/manager', form);
            ...
            if (apiErr?.status === 422 && apiErr?.data?.errors) {
                const mapped: Partial<Record<keyof ManagerFormData, string>> = {};
                Object.entries(apiErr.data.errors).forEach(([key, msgs]) => {
                    mapped[key as keyof ManagerFormData] = msgs[0];
                });
                setErrors(mapped);
            } else {
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-base">
                        ...
                        {editData ? 'Edit Manager' : 'Form Tambah Manager'}
                    </DialogTitle>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                    <div className="space-y-1.5">
                        ...
                        <Input
                            id="nama_manager"
                            value={form.nama_manager}
                            onChange={(e) => handleChange('nama_manager', e.target.value)}
                            placeholder="Nama manager..."
                            autoFocus
                            className={errors.nama_manager ? 'border-destructive focus-visible:ring-destructive' : ''}
                        />
                        {errors.nama_manager && (
                            <p className="text-xs text-destructive">{errors.nama_manager}</p>
                        )}
                    </div>
...
function DeleteDialog({ open, manager, onClose, onSuccess }: DeleteDialogProps) {
    const [loading, setLoading] = useState(false);
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            await apiDelete(`/manager/${manager.id}`);
            toast.success('Manager berhasil dihapus.');
            onSuccess();
            onClose();
        } catch {
            toast.error('Gagal menghapus manager. Silakan coba lagi.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <DialogContent className="sm:max-w-sm">
                ...
                <div className="space-y-3 py-2">
                    <p className="text-sm text-muted-foreground">
                        Apakah Anda yakin ingin menghapus manager{' '}
                        <span className="font-semibold text-foreground">"{manager?.nama_manager}"</span>?
                    </p>
...
export default function ManagerIndex() {
    ...
    const columns: ColumnDef<Manager>[] = [
        ...
        },
        {
            ...
            label: 'NAMA MANAGER',
            sortable: true,
            render: (row) => <span className="font-medium">{row.nama_manager}</span>
        },
        {
            id: 'npp',
            label: 'NPP',
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-6 p-4 md:p-6">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">
                            Kelola data manager dalam organisasi.
                        </p>
                    </div>
                    <Button
                        onClick={openCreate}
                ...
                <ServerDataTable<Manager>
                    endpoint="/manager/datatable"
                    columns={columns}
                    refreshTrigger={refreshTrigger}
                    emptyStateIcon={<Briefcase className="h-7 w-7" />}
                    emptyStateTitle="Tidak ada data"
                    emptyStateMessage="Belum ada manager yang ditambahkan."
                />
            </div>
